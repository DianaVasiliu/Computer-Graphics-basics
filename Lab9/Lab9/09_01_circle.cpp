...
void CreateVBO(void)
{
	...
	for (int i = 0; i < NR_POINTS; i++)
	{
		...
		theta = 2 * PI * i / NR_POINTS;
		
		 /*
		// CIRCLE
		vertex_coords[i] =
			glm::vec4(	cx + radius * cos(theta),
						cy + radius * sin(theta),
						0.0f, 1.0f);
		 */
		...
		a = 15;
		vertex_coords[i] =
			glm::vec4(	a * (2 * cos(theta) + cos(2 * theta)),
						a * (2 * sin(theta) - sin(2 * theta)),
						0.0f, 1.0f);
		 */
		...
		a = 50; b = 10;
		vertex_coords[i] =
			glm::vec4(	(a - b) * cos(theta) + b * cos((a/b - 1) * theta),
						(a - b) * sin(theta) - b * sin((a/b - 1) * theta),
						0.0f, 1.0f);
		 */

		 /*
		// INVOLUTE OF A CIRCLE
		// NOTE: make sure you user GL_LINE_STRIP in RenderFunction, not GL_LINE_LOOP
		a = 1;
		theta = i / 5;
		vertex_coords[i] =
			glm::vec4(	a * (cos(theta) + theta * sin(theta)),
		...
		// a = 20; b = 5; c = 17;
		vertex_coords[i] =
			glm::vec4(	(a - b) * cos(theta) + c * cos((a / b - 1) * theta),
						(a - b) * sin(theta) - c * sin((a / b - 1) * theta),
						0.0f, 1.0f);
		// */
	};
